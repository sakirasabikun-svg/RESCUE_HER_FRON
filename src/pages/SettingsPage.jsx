...
function SettingsPage({ isDarkMode }) {
  // ১. ডার্ক মোড ডিটেকশন (যদি প্রপস না আসে, তবে বডি ক্লাস চেক করবে)
  const [localDark, setLocalDark] = useState(false);

  useEffect(() => {
    // আপনার প্রোজেক্টে যদি <html> বা <body> ট্যাগ-এ 'dark' ক্লাস যোগ করা হয়
    const checkTheme = () => {
      const isDarkClass = document.documentElement.classList.contains('dark') || 
                          document.body.classList.contains('dark') ||
                          document.body.classList.contains('dark-mode');
      setLocalDark(isDarkClass);
    };

    checkTheme();

    // থিম চেঞ্জ অবজার্ভ করার জন্য (যদি প্রয়োজন হয়)
  ...
  // যদি প্রপস থেকে থিম আসে তবে সেটি ব্যবহার করবে, নয়তো লোকাল ডিটেকশন
  const isDark = isDarkMode !== undefined ? isDarkMode : localDark;

  // ২. ডার্ক এবং লাইট মোডের জন্য কালার প্যালেট
  const theme = {
    ...
    switchInactive: isDark ? '#475569' : '#cbd5e1',
    scannerBg: isDark ? '#2d220e' : '#fffbeb',
    scannerBorder: isDark ? '#453007' : '#fef3c7',
  };

  // স্লাইডার এবং টাইমার স্টেট
  const [radius, setRadius] = useState(2); 
  const [countdown, setCountdown] = useState(3); 

  // আইওএস স্টাইল সুইচের স্টেটসমূহ
  const [emailAlert, setEmailAlert] = useState(true);
  const [liveTracking, setLiveTracking] = useState(true);
  const [fakeBattery, setFakeBattery] = useState(false);
  ...
  const toggleSwitchStyle = (isActive) => ({
    width: '46px',
    height: '24px',
    backgroundColor: isActive ? '#00bfa5' : theme.switchInactive, 
    borderRadius: '100px',
    position: 'relative',
    cursor: 'pointer',
    transition: 'background-color 0.2s ease',
  ...
  return (
    <div style={{ padding: '40px 35px', fontFamily: 'system-ui, -apple-system, sans-serif', color: theme.textMain, backgroundColor: theme.bg, minHeight: '100vh', textAlign: 'left', transition: 'background-color 0.3s, color 0.3s' }}>
      ...
      {/* HEADER TITLE */}
      <div style={{ marginBottom: '35px' }}>
        <h2 style={{ fontSize: '28px', fontWeight: '900', color: theme.textMain, marginBottom: '6px', letterSpacing: '-0.5px' }}>Central System Configurations</h2>
        <p style={{ fontSize: '14px', color: theme.textSub, margin: 0, fontWeight: '500' }}>Configure real-time safety thresholds, automated communication nodes, and duress parameters.</p>
      </div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '25px' }}>
        ...
        {/* CARD 1: EMERGENCY RADIUS & TRIGGERS */}
        <div style={{ flex: '1 1 calc(50% - 25px)', minWidth: '320px', backgroundColor: theme.cardBg, padding: '28px', borderRadius: '20px', border: `1px solid ${theme.border}`, boxShadow: '0 10px 25px -5px rgba(15, 23, 42, 0.01)', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', transition: 'background-color 0.3s, border 0.3s' }}>
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '25px' }}>
              <h3 style={{ fontSize: '15px', fontWeight: '800', color: theme.textMain, margin: 0, textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                Perimeter Settings
              </h3>
            ...
            <div style={{ marginBottom: '25px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                <label style={{ fontSize: '13px', fontWeight: '700', color: theme.textLabel }}>Geo-fence Safety Radius</label>
                <span style={{ fontSize: '12px', fontWeight: '800', color: '#ef4444', backgroundColor: '#fee2e2', padding: '3px 10px', borderRadius: '6px' }}>{radius} KM</span>
              </div>
              <input 
                type="range" 
                ...
                style={{ width: '100%', accentColor: '#ef4444', cursor: 'pointer' }}
              />
              <span style={{ fontSize: '11px', color: theme.textSub, display: 'block', marginTop: '6px', fontWeight: '500', lineHeight: '1.4' }}>Defines the critical perimeter boundary for real-time guardian network node updates.</span>
            </div>

            {/* কাউন্টডাউন ড্রপডাউন */}
            <div style={{ marginBottom: '5px' }}>
              <label style={{ display: 'block', fontSize: '13px', fontWeight: '700', color: theme.textLabel, marginBottom: '8px' }}>SOS Transmission Delay Buffer</label>
              <select 
                value={countdown} 
                onChange={(e) => setCountdown(e.target.value)}
                style={{ width: '100%', padding: '12px', borderRadius: '10px', border: `1px solid ${theme.inputBorder}`, fontSize: '13px', backgroundColor: theme.inputBg, cursor: 'pointer', outline: 'none', color: theme.textMain, fontWeight: '500', transition: 'background-color 0.3s, color 0.3s, border 0.3s' }}
              >
                <option value="0">Instant Routing (Zero Latency Bypass)</option>
                <option value="3">3 Seconds Verification Buffer</option>
                <option value="5">5 Seconds Verification Buffer</option>
              ...
              <span style={{ fontSize: '11px', color: theme.textSub, display: 'block', marginTop: '6px', fontWeight: '500', lineHeight: '1.4' }}>Grace period allocated to terminate accidental distress triggers prior to network broadcast.</span>
            </div>
          </div>
        </div>
        ...
        <div style={{ flex: '1 1 calc(50% - 25px)', minWidth: '320px', backgroundColor: theme.cardBg, padding: '28px', borderRadius: '20px', border: `1px solid ${theme.border}`, boxShadow: '0 10px 25px -5px rgba(15, 23, 42, 0.01)', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', transition: 'background-color 0.3s, border 0.3s' }}>
          <div>
            ...
            {/* সুইজার ১ */}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: '16px', borderBottom: `1px solid ${theme.border}`, marginBottom: '16px' }}>
              <div style={{ textAlign: 'left', paddingRight: '15px' }}>
                <span style={{ fontSize: '14px', fontWeight: '600', color: theme.textLabel, display: 'block', marginBottom: '2px' }}>Automated Emergency Broadcasts</span>
                <span style={{ fontSize: '11px', color: theme.textSub, fontWeight: '500', display: 'block', lineHeight: '1.3' }}>Directly transmit location data mapping metrics to saved emergency channels via SMTP relays.</span>
              </div>
              <button style={toggleSwitchStyle(emailAlert)} onClick={() => setEmailAlert(!emailAlert)}>
                <span style={toggleCircleStyle(emailAlert)}></span>
              </button>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: '16px', borderBottom: `1px solid ${theme.border}`, marginBottom: '16px' }}>
              <div style={{ textAlign: 'left', paddingRight: '15px' }}>
                <span style={{ fontSize: '14px', fontWeight: '600', color: theme.textLabel, display: 'block', marginBottom: '2px' }}>Background Coordinate Streaming</span>
                <span style={{ fontSize: '11px', color: theme.textSub, fontWeight: '500', display: 'block', lineHeight: '1.3' }}>Preserve continuous server synchronization protocols even when the device gateway is minimized.</span>
              </div>
              <button style={toggleSwitchStyle(liveTracking)} onClick={() => setLiveTracking(!liveTracking)}>
                <span style={toggleCircleStyle(liveTracking)}></span>
              </button>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ textAlign: 'left', paddingRight: '15px' }}>
                <span style={{ fontSize: '14px', fontWeight: '600', color: theme.textLabel, display: 'block', marginBottom: '2px' }}>Cloaking Parameter Simulation</span>
                <span style={{ fontSize: '11px', color: theme.textSub, fontWeight: '500', display: 'block', lineHeight: '1.3' }}>Simulate alternative hardware profiles to disrupt analytics if the system encounters physical inspection.</span>
              </div>
              <button style={toggleSwitchStyle(fakeBattery)} onClick={() => setFakeBattery(!fakeBattery)}>
                <span style={toggleCircleStyle(fakeBattery)}></span>
              </button>
        ...
        {/* CARD 3: HIGH-TECH DURESS PIN CONFIGURATOR */}
        <div style={{ flex: '1 1 100%', backgroundColor: theme.cardBg, padding: '28px', borderRadius: '20px', border: `1px solid ${theme.border}`, boxShadow: '0 10px 25px -5px rgba(15, 23, 42, 0.01)', marginTop: '5px', display: 'flex', gap: '30px', flexWrap: 'wrap', alignItems: 'center', transition: 'background-color 0.3s, border 0.3s' }}>
          
          <div style={{ flex: '1 1 400px', borderLeft: '4px solid #f59e0b', paddingLeft: '16px', textAlign: 'left' }}>
            <h3 style={{ fontSize: '16px', fontWeight: '800', color: theme.textMain, marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
              Duress Stealth Protocol Matrix
            </h3>
            <p style={{ fontSize: '13px', color: theme.textSub, margin: 0, fontWeight: '500', lineHeight: '1.5' }}>
              In scenarios where threat actors compel system decryption, entering this specialized secondary PIN masks the core workspace. The terminal will initialize an uncompromised backup container dashboard while silently deploying background distress signals to all network nodes.
            </p>
          </div>

          {/* STEALTH MATRIX SCANNER */}
          <div style={{ width: '60px', height: '60px', backgroundColor: theme.scannerBg, borderRadius: '16px', border: `1px solid ${theme.scannerBorder}`, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden', transition: 'background-color 0.3s, border 0.3s' }}>
            <div className="animate-scan" style={{ position: 'absolute', width: '80%', height: '2px', backgroundColor: '#f59e0b', left: '10%' }}></div>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#f59e0b" strokeWidth="2">
              <rect x="3" y="11" width="18" height="11" rx="2" ry="2"></rect>
              <path d="M7 11V7a5 5 0 0 1 10 0v4"></path>
          ...
          <div style={{ flex: '1 1 100%', marginTop: '5px' }}>
            <form onSubmit={handleSavePin} style={{ display: 'flex', gap: '15px', alignItems: 'flex-end', flexWrap: 'wrap' }}>
              <div style={{ flex: '1 1 240px', textAlign: 'left' }}>
                <label style={{ display: 'block', fontSize: '12px', fontWeight: '700', color: theme.textLabel, marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                  Configure 4-Digit Duress PIN
                </label>
                <input 
                  type="password" 
                  ...
                  value={duressPin}
                  onChange={(e) => setDuressPin(e.target.value.replace(/\D/g, ''))} 
                  style={{ width: '100%', padding: '12px 14px', borderRadius: '10px', border: `1px solid ${theme.inputBorder}`, fontSize: '15px', letterSpacing: '6px', boxSizing: 'border-box', outline: 'none', color: theme.textMain, backgroundColor: theme.inputBg, fontWeight: '700', transition: 'background-color 0.3s, color 0.3s, border 0.3s' }}
                />
              </div>
              <button 
                type="submit" 
                style={{ padding: '13px 28px', backgroundColor: isDark ? '#f1f5f9' : '#0f172a', color: isDark ? '#0f172a' : 'white', border: 'none', borderRadius: '10px', fontSize: '13px', fontWeight: '700', cursor: 'pointer', transition: 'background-color 0.2s', textTransform: 'uppercase', letterSpacing: '0.5px' }}
                onMouseEnter={(e) => e.target.style.backgroundColor = isDark ? '#e2e8f0' : '#1e293b'}
                onMouseLeave={(e) => e.target.style.backgroundColor = isDark ? '#f1f5f9' : '#0f172a'}
              >
                Update Duress Matrix
              </button>
            </form>
